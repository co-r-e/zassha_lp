...
export function Button({ href, children, variant = "primary", targetBlank = false, className }: ButtonProps) {
  const baseClasses =
  ...
  const composedClasses = `${baseClasses} ${variantClasses}${className ? ` ${className}` : ""}`;
